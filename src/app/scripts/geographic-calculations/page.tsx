...
const MapComponent = dynamic(
  () => import('./components/MapComponent').then(mod => {
    ...
    return () => <div className="w-full h-full flex items-center justify-center bg-red-100 text-red-500">Xəritə yüklənmədi</div>;
  }),
  {
    ssr: false,
    loading: () => (
      <AixerisLoading label="Xəritə yüklənir..." />
    )
  }
);
...
export default function GeographicCalculationsPage() {
  ...
  const handleDrawCreated = useCallback((e: any) => {
    try {
      ...
      setError(`Çizim yaradılma xətası: ${err.message || 'Naməlum xəta'}`);
    }
  }, []);
  ...
  const clearDrawings = useCallback(() => {
    try {
      ...
      setError(`Çizimləri təmizləmə xətası: ${err.message || 'Naməlum xəta'}`);
    }
  }, []);
  ...
  const calculateArea = useCallback(() => {
    try {
      if (drawnItems.length === 0) {
        setCalculationResult("Zəhmət olmasa bir sahə çəkin");
        return;
      }
      ...
      if (polygons.length === 0) {
        setCalculationResult("Sahəni hesablamaq üçün bir çoxbucaqlı çəkməlisiniz");
        return;
      }
      ...
      setError(`Sahə hesablanması xətası: ${err.message || 'Naməlum xəta'}`);
      setCalculationResult("Hesablama zamanı xəta baş verdi");
    }
  }, [drawnItems]);
  ...
  const calculateDistance = useCallback(() => {
    try {
      if (drawnItems.length === 0) {
        setCalculationResult("Zəhmət olmasa bir xətt çəkin");
        return;
      }
      ...
      if (lines.length === 0) {
        setCalculationResult("Məsafəni hesablamaq üçün bir xətt çəkməlisiniz");
        return;
      }
      ...
      setError(`Məsafə hesablanması xətası: ${err.message || 'Naməlum xəta'}`);
      setCalculationResult("Hesablama zamanı xəta baş verdi");
    }
  }, [drawnItems]);
  ...
  const createBuffer = useCallback(() => {
    try {
      if (drawnItems.length === 0) {
        setCalculationResult("Zəhmət olmasa bir geometri çəkin");
        return;
      }
      ...
      setCalculationResult(`${bufferDistance} km tampon zona yaradıldı`);
    } catch (err: any) {
      console.error("Buffer creation error:", err);
      setError(`Tampon zonanın yaradılması xətası: ${err.message || 'Naməlum xəta'}`);
      setCalculationResult("Tampon zona yaradılarkən xəta baş verdi");
    }
  ...
  const calculateIntersection = useCallback(() => {
    try {
      if (drawnItems.length < 2) {
        setCalculationResult("Kəsişmə üçün ən az iki geometri çəkməlisiniz");
        return;
      }
      ...
      if (!isPolygon1 || !isPolygon2) {
        setCalculationResult("Kəsişmə analizi üçün iki çoxbucaqlı (polygon) geometri tələb olunur.");
        return;
      }
      ...
      if (!intersection) {
        setCalculationResult("Geometriyalar kəsişmir");
        return;
      }
      ...
      setError(`Kəsişmə hesablanması xətası: ${err.message || 'Naməlum xəta'}`);
      setCalculationResult("Kəsişmə hesablana bilmədi. Geometri tiplərini yoxlayın.");
    }
  }, [drawnItems]);
  ...
  const calculateUnion = useCallback(() => {
    try {
      if (drawnItems.length < 2) {
        setCalculationResult("Birləşmə üçün ən az iki geometri çəkməlisiniz");
        return;
      }
      ...
      if (!isPolygon1 || !isPolygon2) {
        setCalculationResult("Birləşmə analizi üçün iki çoxbucaqlı (polygon) geometri tələb olunur.");
        return;
      }
      ...
      if (!union) {
        setCalculationResult("Geometriyalar birləşdirilə bilmədi");
        return;
      }
      ...
      setError(`Birləşmə hesablanması xətası: ${err.message || 'Naməlum xəta'}`);
      setCalculationResult("Birləşmə hesablana bilmədi. Geometri tiplərini yoxlayın.");
    }
  }, [drawnItems]);
  ...
  const performCalculation = useCallback(() => {
    try {
      ...
      setError(`Hesablama xətası: ${err.message || 'Naməlum xəta'}`);
    }
  ...
  const handleSetActiveCalculation = useCallback((type: string) => {
    try {
      ...
      setError(`Hesablama tipini ayarlama xətası: ${err.message || 'Naməlum xəta'}`);
    }
  }, [error]);
  ...
  const handleGeometryInfo = useCallback((info: any) => {
    ...
    if (info && info.deleted) {
      ...
      setCalculationResult(`${info.count} geometri uğurla silindi`);
      ...
      <div className="flex-1 flex overflow-hidden relative z-10">
              ...
              <div className="space-y-5">
                <div className="bg-gradient-to-br from-[#142F47]/70 to-[#0A1A2F]/70 p-4 rounded-xl border border-[#00B4A2]/30 shadow-xl">
                  <h2 className="text-lg font-semibold mb-4 text-transparent bg-clip-text bg-gradient-to-r from-[#00E5CC] to-[#8B6FFF] flex items-center">
                    <CalculationIcon />
                    <span className="ml-2">Hesablama Alətləri</span>
                  </h2>

                  <div className="grid grid-cols-1 gap-2.5">
                    {[
                      { id: 'area', label: 'Sahəni hesabla', icon: <svg className="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z" /></svg> },
                      { id: 'distance', label: 'Məsafəni ölç', icon: <svg className="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 13l-7 7-7-7m14-8l-7 7-7-7" /></svg> },
                    ].map(tool => (
                      <button
                        key={tool.id}
                ...
                {(calculationResult || (selectedGeometryInfo && selectedGeometryInfo.text)) && (
                  <div className="bg-gradient-to-br from-[#142F47]/70 to-[#0A1A2F]/70 p-4 rounded-xl border border-[#00B4A2]/30 shadow-xl">
                    <h2 className="text-lg font-semibold mb-3 text-transparent bg-clip-text bg-gradient-to-r from-[#00E5CC] to-[#8B6FFF]">
                      {selectedGeometryInfo && selectedGeometryInfo.text ? 'Geometri Məlumatı' : 'Nəticə'}
                    </h2>
                    ...
                    {selectedGeometryInfo && selectedGeometryInfo.type === 'multiple' && (
                      <div className="mt-3 text-gray-400 text-xs space-y-1">
                        <p className="flex items-center"><svg className="w-3 h-3 mr-1.5 text-[#00E5CC]" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd"></path></svg>Bir neçə geometri seçilib. Silmək üçün xəritədəki zibil qutusundan istifadə edin.</p>
                        <p className="flex items-center"><svg className="w-3 h-3 mr-1.5 text-[#00E5CC]" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd"></path></svg>Tək geometri məlumatı üçün CTRL olmadan klikləyin.</p>
                      </div>
                    )}
                  </div>
                )}
                ...
                {error && (
                  <div className="bg-red-700/20 border border-red-500/60 rounded-xl p-4 shadow-lg">
                    <div className="flex items-start">
                      ...
                      <div>
                        <h3 className="font-semibold text-red-300 text-md">Xəta baş verdi</h3>
        ...
        <div className="flex-1 relative">
          ...
          <div className="h-full">
            <Suspense fallback={<AixerisLoading label="Xəritə yüklənir..." />}>
            ...
            <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-[#142F47]/90 text-white px-3 py-1.5 rounded-lg shadow-lg border border-[#00B4A2]/30 text-sm max-w-md text-center z-30">
              <p>
                {selectedGeometryInfo.type === 'multiple'
                  ? `${selectedGeometryInfo.count} geometri seçildi. Silmək üçün zibil qutusu ikonundan istifadə edin.`
                  : 'Geometri məlumatları yan paneldə göstərilir.'}
              </p>
            </div>
          )}
        </div>
      ...
      <div className="absolute bottom-4 right-4 z-30 opacity-0 animate-fadeInUp animation-delay-2000" style={{ animationDelay: '2s', animationFillMode: 'forwards' }}>
        <div className="bg-[#0A1A2F]/80 backdrop-blur-sm p-3 rounded-lg shadow-xl border border-[#00B4A2]/40 text-xs text-gray-200 hover:shadow-[#00E5CC]/20 transition-shadow">
          <div className="flex items-center space-x-2">
            ...
            <span>Yuxarı sol küncdəki alətlərlə çəkə bilərsiniz.</span>
          </div>
        </div>
      </div>
    </div>
