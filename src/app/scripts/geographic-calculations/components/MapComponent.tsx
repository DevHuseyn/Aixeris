...
const MapComponent = memo(function MapComponent({ onDrawCreated, drawnItems, activeCalculation, onGeometryInfo }: MapComponentProps) {
  ...
  const formatGeometryInfo = (geoJSON: any) => {
    try {
      if (!geoJSON || !geoJSON.geometry) return "Geometri bilgisi alınamadı";
      ...
      switch (geometryType) {
        ...
        case 'MultiPolygon':
          ...
          infoText = `<b>Geometri Tipi:</b> ${geometryType}<br/><b>Alan:</b> ${area < 10000 ? area.toFixed(2) + ' m²' : (area / 1000000).toFixed(4) + ' km²'}`;
          break;
        case 'LineString':
        case 'MultiLineString':
          const length = turf.length(geoJSON, { units: 'kilometers' });
          infoText = `<b>Geometri Tipi:</b> ${geometryType}<br/><b>Uzunluk:</b> ${length < 1 ? (length * 1000).toFixed(2) + ' m' : length.toFixed(3) + ' km'}`;
          break;
        case 'Point':
          const coords = geoJSON.geometry.coordinates;
          infoText = `<b>Geometri Tipi:</b> ${geometryType}<br/><b>Koordinatlar:</b> ${coords[1].toFixed(6)}, ${coords[0].toFixed(6)}`;
          break;
        default:
          infoText = `<b>Geometri Tipi:</b> ${geometryType}`;
      }
      ...
      if (geoJSON.properties && Object.keys(geoJSON.properties).length > 0) {
        ...
        if (propDetails) {
          infoText += `<br/><br/><b>Özellikler:</b><br/>${propDetails}`;
        }
      }
      
      return infoText;
    } catch (error) {
      console.error("Geometri bilgisi formatlanırken hata:", error);
      return "Geometri bilgisi alınamadı";
    }
  };
    ...
    try {
      ...
      map.on(L.Draw.Event.DELETED, (e: any) => {
        try {
          ...
          if (onGeometryInfo) {
            onGeometryInfo({
              text: `${deletedCount} geometri silindi`,
              deleted: true,
              count: deletedCount,
              ...
              drawHandlerRef.current._onMouseMove = function(e: L.LeafletMouseEvent) {
                ...
                if (distance < 50) {
                  ...
                  if (startMarker) {
                    ...
                    if (this._tooltip) {
                      this._tooltip.updateContent({ text: 'Çizimi kapatmak için ilk noktaya tıklayın' });
                    }
                  }
                } else {
                ...
                if (selectedLayer) {
                  try {
                    ...
                    switch (geometryType) {
                      ...
                      case 'MultiPolygon':
                        const area = turf.area(geoJSON);
                        infoText = `Polygon | Alan: ${area < 10000 ? area.toFixed(2) + ' m²' : (area / 1000000).toFixed(4) + ' km²'}`;
                        break;
                      case 'LineString':
                      case 'MultiLineString':
                        const length = turf.length(geoJSON, { units: 'kilometers' });
                        infoText = `Çizgi | Uzunluk: ${length < 1 ? (length * 1000).toFixed(2) + ' m' : length.toFixed(3) + ' km'}`;
                        break;
                      case 'Point':
                        const coords = geoJSON.geometry.coordinates;
                        infoText = `Nokta | Koordinatlar: ${coords[1].toFixed(6)}, ${coords[0].toFixed(6)}`;
                        break;
                      default:
                        infoText = `Geometri Tipi: ${geometryType}`;
                    }
  ...
  const deleteSelectedGeometries = () => {
    try {
      ...
      if (selectedLayersRef.current.length > 0) {
        ...
        if (onGeometryInfo) {
          onGeometryInfo({
            text: `${selectedFeatures.length} geometri silindi`,
            deleted: true,
    ...
    <div className="relative w-full h-full">
      ...
      <div className="absolute top-4 right-4 z-[1000] flex flex-col space-y-2">
        ...
        <button
          ...
          className={`p-2 rounded-lg shadow-lg flex items-center justify-center transition-all ${
            selectionModeActive 
              ? 'bg-[#FF4500] text-white border-2 border-white' 
              : 'bg-white text-[#142F47] hover:bg-gray-100'
          }`}
          title={selectionModeActive ? 'Seçim Modunu Kapat' : 'Geometrileri Seç'}
        >
